...
export abstract class CardViewImpl<T> extends ViewGroupImpl<T>{
	...
	@decorate(Type(() => CommandAttr))
	@decorate(Expose({ name: "shadowOffset" }))
	shadowOffset!:CommandAttr<string>| undefined;
	...
	public setShadowOffset(value : string) : T {
		this.resetIfRequired();
		if (this.shadowOffset == null || this.shadowOffset == undefined) {
			this.shadowOffset = new CommandAttr<string>();
		}
		
		this.shadowOffset.setSetter(true);
		this.shadowOffset.setValue(value);
		this.orderSet++;
		this.shadowOffset.setOrderSet(this.orderSet);
		return this.thisPointer;
	}
